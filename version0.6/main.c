...
int main(int argc, char **argv)
{
	...
	FILE* f = fopen("save_location.conf", "w"); //dont forget to fclose(f) at some point
	if(check_conf_file(f) == 1)
	{
		/* Replace save location with what is in the conf file*/
		int i;
		for(i = 0; i != EOF; i++)
		{
			/* Store to save_location from conf_file*/
		}
	...
	else 
	{
		...
		create_file(save_location);
		create_conf_file(save_location, f);
	}
		...
		if(first_run == 1)
		{
			...
			if(norm_L2 > 3000)
			{
				...
				/*if(i > 5)
				{
					...
					save_image(frame); */
				sleep(0.5); //delay 500 ms not to fill dropbox
